		...
		DELETE FROM loot where id=${req.params.id}`)
			.then(result => {
				if(result[1].rowCount >= 1) {
					res.send({ 
						message: 'Deletion successful, the following item was deleted from the database:', 
						deletedItem: deletedItem.rows[0] 
					}); //would like this to be less messy if possible?
				}
				else {
					res.send({ 
						error: `No item matching the loot ID of ${req.params.id} was found. No item was deleted.` 
					});
				}
			})
			// eslint-disable-next-line no-console
